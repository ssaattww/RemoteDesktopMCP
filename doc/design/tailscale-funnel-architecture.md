...
- `DATA_DIR`、認証設定、監査ログを RemoteDesktopMCP の許可 root と `@wonderwhy-er/desktop-commander` のローカル許可範囲のどちらにも含めない。
...
- 現在のファイル探索は `readdir` / `stat`、プロセス起動は直接 `spawn` を使用しているが、これらは暫定 `scaffold` とし、実装時に `@wonderwhy-er/desktop-commander` の MCP ツール委譲へ置き換えて直接実装を削除する。
