...
この設定を変更する MCP ツール、HTTP 管理 API、Funnel 経由の管理画面は作らない。
許可ユーザー設定と OAuth/OIDC の秘密情報は、RemoteDesktopMCP の管理側プロセスが使用する OS ユーザーだけが読み書きできるようにする。
Desktop Commander と `process_start` のコマンドを実行する専用ユーザーには、この設定の読み書き権限を与えない。

### ローカル秘密情報と実行環境

管理側プロセスが保持する秘密情報を、Desktop Commander や起動したコマンドの環境変数へ渡さない。
アクセストークン、リフレッシュトークン、OAuth/OIDC の秘密情報、ノード認証用PSKなどは管理側だけで保持する。
監査ログ、認証設定、`DATA_DIR`、サービス設定なども、実行側ユーザーから書き換えられない OS アクセス権で保護する。

`process_start` を有効にする前に、実際に配備した実行ユーザーで、許可した作業用ディレクトリへはアクセスでき、これらの保護対象へはアクセスできないことを確認する。
実行ユーザーの分離や OS のアクセス権を確認できない場合は `process_start` を公開しない。
