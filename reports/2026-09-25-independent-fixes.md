...
これらは修正候補のローカル証拠であり、同じコミットの両 OS CI と通常 reviewer の限定確認を次のゲートとする。
