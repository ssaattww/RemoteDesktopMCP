...
候補 `00e693923f7ce8709386b089707fb9eefda32d76` は commit・push 済み。PR CI run `36046213583` の Ubuntu job `107790302517` は24/24成功、Windows job `107790302868` は23/24で失敗した。
失敗した bigint identity fixture は起動直後の pin 一覧に必ず要素があると仮定し、空配列の先頭を path として使っていた。
通常 reviewer は、初期化末尾の sole-link pin 清掃後に空となることは正当であり、保護違反ではなく fixture のライフサイクル前提の誤りと確認した。
