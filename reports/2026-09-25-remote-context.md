...
再開時、既存 state の ACL を親からの限定継承から明示保護へ移行した。現在の Windows ユーザー、SYSTEM、管理者の3主体だけとし、継承を無効化した。処理前後の SHA256 比較でファイル内容が変わっていないことを確認した。
新しい保存領域検査で、既存 `.env` と管理ディレクトリの所有者が現在のユーザーと一致せず拒否された。両方の所有者を現在のユーザーへ変更し、許可主体は増やしていない。`.env` の SHA256 は不変で、作業フォルダの親・設定ファイル・管理ディレクトリ・本人登録 state の検査は全て成功した。
