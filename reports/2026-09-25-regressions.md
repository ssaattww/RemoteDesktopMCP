...
| RDMCP-DR-003 | A hard link to the original config remains rejected after an atomic non-secret `config.json` replacement, and a hard link to the replacement inode is also rejected. Content search and a swapped upload-temp pathname cannot reach either protected config identity; traversal and resolved DATA_DIR overlap are rejected. |
