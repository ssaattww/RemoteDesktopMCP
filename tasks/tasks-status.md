...
| F01 | P4 | Google OIDC、CIMD、refresh token | T08 | L | 公開用認証設計と一致し、実アカウントとの接続を確認 | 実装済み、レビューと実認証は未完了 |
| F01a | P4 | 公開認証の回帰・再起動・拒否試験 | F01 | M | 誤ユーザー、改ざん、再利用、期限、クライアントと対象資源の照合を検証 | 追加9試験成功、既存の失敗1件は修正後の個別試験成功、全件確認待ち |
| F01b | P4 | 設定補助と Windows 起動手順 | F01 | M | 秘密を公開せず設定し、継続起動と停止・再起動を確認 | 設定補助・手順作成済み、本人登録待ち |
| F02 | P4 | ノード間相互認証・複数PC経路・再接続 | T08 | L | 登録2台以上、切断・世代交代・再送・転送中継の試験が合格 | 後続 |
| F03 | P4 | 単一 PC の Tailscale Funnel と ChatGPT 公開接続検証 | F01,F01a,F01b | M | 実際の公開経路から認証と操作、再起動後の接続を確認 | 環境確認済み、認証設定待ち |
| F04 | P4 | 公開接続の通常・独立レビュー | F01a,F01b | M | Sol / high で公開用変更を確認し、F03 の実運用結果と未検証事項を区別 | 通常レビュー準備中 |
