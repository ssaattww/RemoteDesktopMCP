...
| F01 | P4 | Google OIDC、CIMD、refresh token | T08 | L | 公開用認証設計と一致し、実アカウントとの接続を確認 | 実 Google 本人登録成功、通常指摘修正中、ChatGPT 認証待ち |
| F01a | P4 | 公開認証の回帰・再起動・拒否試験 | F01 | M | 誤ユーザー、改ざん、再利用、期限、クライアントと対象資源の照合を検証 | 公開16件と Windows ACL 1件成功、POSIX 1件除外。追加の不足試験を補強済み、修正レビューと全件確認待ち |
| F01b | P4 | 設定補助と Windows 起動手順 | F01 | M | 秘密を公開せず設定し、継続起動と停止・再起動を確認 | 実設定と本人登録済み、保存領域保護を修正中 |
| F01c | P4 | 保存領域保護と既存操作の回帰修正 | F01a,F01b | M | 厳密な権限確認を維持し、既存のプロセス・転送・認証試験と Node 22 全体試験が成功 | 初回8件失敗を修正し全体再確認中。直接レビューの REMOTE-NR-010 は終了待ちの出力取得と合成試験を追加修正 |
| F02 | P4 | ノード間相互認証・複数PC経路・再接続 | T08 | L | 登録2台以上、切断・世代交代・再送・転送中継の試験が合格 | 後続 |
| F03 | P4 | 単一 PC の Tailscale Funnel と ChatGPT 公開接続検証 | F01,F01a,F01b,F01c | M | 実際の公開経路から認証と操作、再起動後の接続を確認 | 実設定済み、全体試験の回帰修正後に公開起動 |
| F04 | P4 | 公開接続の通常・独立レビュー | F01a,F01b,F01c | M | Sol / high で公開用変更を確認し、F03 の実運用結果と未検証事項を区別 | 75197bb で通常9件のコード指摘解消、全体試験が示した F01c の修正確認と独立レビュー待ち |
