...
| T03 | P2 | 単一PCの認証、セッション、ノード、監査、設定 | T02 | M | loopback 限定、未認証拒否、セッション期限と所有者検証、未知ノード拒否 | 実装・回帰試験完了、再レビュー待ち |
| T04 | P2 | Desktop Commander のファイル・プロセス操作への委譲 | T03 | L | 固定版でツール一覧を検証し、検索・読取・部分編集・起動・状態・出力・停止を確認 | 実装・回帰試験完了、再レビュー待ち |
| T05 | P2 | MCP の双方向ファイル転送 | T04 | L | 7転送ツール、サイズと hash 検証、競合保護、中断・期限・清掃を確認 | 実装・回帰試験完了、再レビュー待ち |
| T06 | P2 | 設定例・起動手順・回帰と結合テスト | T05 | M | 初回起動手順が実行可能、旧 download URL を廃止、3指摘の競合 fixture が合格、Windows/Linux CI に検証を配線 | 12件の回帰試験合格、再レビュー待ち |
| T06a | P2 | Desktop Commander の依存監査と修正版への限定更新 | T04 | S | sharp と uuid の報告を調査、互換性確認と実接続・audit 再検証 | 実装・回帰試験完了、再レビュー待ち |
| T07 | P3 | 通常レビューと指摘修正 | T06 | M | Sol / high による必須観点レビューと修正確認、検証報告が揃う | 修正・回帰試験完了、再レビュー待ち |
| T07a | P3 | 設定保護と転送清掃 | T07 | M | DR-003、NR-002、DR-001/002 の不足 fixture を修正・検証 | 修正・回帰試験完了、再レビュー待ち |
| T07b | P3 | 検索完了とプロセス出力・終了監査 | T07 | M | NR-003/004 のページ処理、文字列検索、終了結果を実接続で確認 | 修正・回帰試験完了、再レビュー待ち |
| T07c | P3 | Node22、OAuth、セッションの契約 | T07 | M | NR-001/005/006、実 HTTP の正常・異常経路を検証 | 修正・回帰試験完了、再レビュー待ち |
| T07d | P3 | 再レビューの残存指摘と起動時清掃 | T07 | M | NR-003/004/006/007/008、全検索結果・未照会終了監査・有効セッション一覧・両OS試験・所有証明を確認 | 修正済み、最終試験・Linux CI・再レビュー待ち |
| T07e | P3 | CI で判明した設定実体の置換と委譲失敗 | T07d | M | 再発 DR-003 と NR-009 を修正し、両 OS の同一候補 CI が成功 | 修正中 |
