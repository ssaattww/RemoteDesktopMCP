...
| F01a | P4 | 公開認証の回帰・再起動・拒否試験 | F01 | M | 誤ユーザー、改ざん、再利用、期限、クライアントと対象資源の照合を検証 | 追加9試験成功、既存の失敗1件は修正後の個別試験成功、全件確認待ち |
...
| F04 | P4 | 公開接続の通常・独立レビュー | F01a,F01b | M | Sol / high で公開用変更を確認し、F03 の実運用結果と未検証事項を区別 | 通常レビュー fail、REMOTE-NR-001〜009 を修正中 |
